    ...
    public static class Helper
    {
        ...
        internal static byte[] AES256Decrypt(this byte[] block, byte[] key)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                ...
                {
                    return TR.Exit(decryptor.TransformFinalBlock(block, 0, block.Length));
                }
            }
        }
        ...
        internal static byte[] AES256Encrypt(this byte[] block, byte[] key)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                ...
                {
                    return TR.Exit(encryptor.TransformFinalBlock(block, 0, block.Length));
                }
            }
        }
        ...
        internal static byte[] AesDecrypt(this byte[] data, byte[] key, byte[] iv)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                ...
                {
                    return TR.Exit(decryptor.TransformFinalBlock(data, 0, data.Length));
                }
            }
        }
        ...
        internal static byte[] AesEncrypt(this byte[] data, byte[] key, byte[] iv)
        {
            ...
            using (Aes aes = Aes.Create())
            {
                ...
                {
                    return TR.Exit(encryptor.TransformFinalBlock(data, 0, data.Length));
                }
            }
        }
        ...
        public static byte[] Base58CheckDecode(this string input)
        {
            ...
            return TR.Exit(buffer.Take(buffer.Length - 4).ToArray());
        }
        ...
        {
            TR.Enter();
            return TR.Exit(_ripemd160.Value.ComputeHash(value.ToArray()));
        }
        ...
        public static uint Murmur32(this IEnumerable<byte> value, uint seed)
        {
            ...
            {
                return TR.Exit(murmur.ComputeHash(value.ToArray()).ToUInt32(0));
            }
        }
        ...
        {
            TR.Enter();
            return TR.Exit(_sha256.Value.ComputeHash(value.ToArray()));
        }
        ...
        {
            TR.Enter();
            return TR.Exit(_sha256.Value.ComputeHash(value, offset, count));
        }
